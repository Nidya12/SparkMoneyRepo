...
public class BasePage {
    ...
    WebDriverWait wait = new WebDriverWait(AppDriver.getCurrentDriver(), Duration.ofSeconds(45));

    protected WebElement waitForElement(By byLocator){
       return wait.until(ExpectedConditions.presenceOfElementLocated(byLocator));
    }



    protected WebElement getElement(By byLocator){
        return AppDriver.getCurrentDriver().findElement(byLocator);
    }

    protected List<WebElement> getElements(By byLocator){
    ...
    protected void type(By byLocator, String text){
        getElement(byLocator).sendKeys(text);
    }
    ...
    protected void waitNclick(By byLocator){
        waitForElement(byLocator).click();
    }
    ...
    protected String getText(By byLocator){
        ...
        if (AppDriver.getCurrentDriver() instanceof AndroidDriver) {
            str = getElement(byLocator).getText();
    ...
    protected String getAttribute(By byLocator, String attr){
        return waitForElement(byLocator).getAttribute(attr);
    }
